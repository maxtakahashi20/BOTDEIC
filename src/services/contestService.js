...
async function createApplicationTicket(guild, config, user, answers) {
  ...
  const nome =
    (answers.dados_pessoais ?? answers.nome_completo ?? user.username)
      .split("\n")[0]
      .trim()
...
async function closeApplicationChannel(
  client,
  config,
  application,
  closedBy,
  ...
  const logsChannelId =
    config.channels?.ticketsLogs ?? config.channels?.generalLogs;
  if (logsChannelId) {
    ...
    if (logsChannel?.isTextBased()) {
      ...
      const logEmbed = new EmbedBuilder()
        .setTitle(
          approved
            ? "✅ Inscrição Aprovada — Canal Encerrado"
            : "❌ Inscrição Reprovada — Canal Encerrado",
        )
        .addFields(
          {
            name: "Candidato",
      ...
      await logsChannel.send({
        ...
        files: [
          {
            attachment: buffer,
            name: `transcript-inscricao-${application.id}.html`,
          },
        ],
      });
    }
...
async function approveApplication(client, config, application, reviewer) {
  ...
  try {
    ...
    await user.send(
      config.messages?.contestApprovedDm ?? "✅ Inscrição aprovada!",
    );
  } catch {
    // DM fechada
  }

  await closeApplicationChannel(
    client,
    config,
    application,
    reviewer.id,
...
async function rejectApplication(client, config, application, reviewer) {
  ...
  try {
    ...
    await user.send(
      config.messages?.contestRejectedDm ?? "❌ Inscrição reprovada.",
    );
  } catch {
    // DM fechada
  }
